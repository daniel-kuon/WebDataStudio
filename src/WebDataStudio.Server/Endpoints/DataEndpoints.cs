            ...
            BrowseRequestDto body, SessionFactory factory, MaskPolicyStore policies, CancellationToken ct) =>
        {
            try
            {
                ...
                await using (session)
                {
                    ...
                    // Borrowed columns, validated the way the GET validates them: a single-column
                    // key of this table, and a real column of the table it points at. An entry
                    // that resolves to nothing is skipped, not an error — a column dropped since
                    // it was borrowed should not take the whole page down.
                    var lookups = new List<ResolvedLookup>();
                    foreach (var spec in (body.Lookups ?? []).Take(8))
                    {
                        ...
                        var targetRef = BrowseQuery.ReferencedRef(key, detail.Ref);
                        ObjectDetail borrowed;
                        try { borrowed = await driver.DescribeAsync(session, targetRef, ct); }
                        catch (Exception) { continue; }

                        var column = borrowed.Columns.FirstOrDefault(c =>
                            c.Name.Equals(wanted, StringComparison.OrdinalIgnoreCase));
                        if (column is null) continue;

                        lookups.Add(new ResolvedLookup(key, $"{key.Columns[0]}.{column.Name}",
                            column, targetRef));
                    }
